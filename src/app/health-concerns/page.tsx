...
import type { HealthConcern, Banner } from '@/types';
...
export default function HealthConcernsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    return concerns.filter((hc) => hc.name.toLowerCase().includes(q));
  }, [concerns, search]);
    ...
    <div className="app-root min-h-screen pb-16 md:pb-0 bg-[#f8fafc]">
      ...
      <main className="mx-auto max-w-7xl px-4 md:px-8 py-5">
            ...
            {banner.imageUrl ? (
              <Link href={(banner.ctaHref || '/products').replace(/&#x2F;/g, '/')} className="block overflow-hidden transition hover:opacity-95">
                <img
                  ...
                  alt={banner.title || 'Health Concerns Banner'}
              ...
              <div className="relative overflow-hidden bg-gradient-to-r from-[#006872] via-[#007a87] to-[#008f9f] p-6 md:p-10 text-white">
                <div className="relative z-10 max-w-2xl space-y-2">
                  ...
                  <h2 className="text-[24px] md:text-[32px] font-extrabold leading-tight">
                    {banner.title || 'Targeted Health & Wellness Solutions'}
                  </h2>
                  {banner.subtitle && (
        ...
        <nav className="mb-4 flex items-center gap-2 text-[12.5px] text-[#64748b]" aria-label="Breadcrumb">
          ...
          <span>/</span>
          <span className="font-semibold text-[#0f172a]">Health Concerns</span>
        </nav>
        ...
        <div className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            ...
            <p className="text-[12.5px] text-[#64748b] mt-0.5">
              {loading ? 'Loading…' : `Targeted health solutions, ayurveda & vitamins for ${concerns.length} wellness needs`}
            </p>
          </div>
        ...
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center rounded-3xl bg-white border border-[#e2e8f0] p-8">
            <Icon name="health_and_safety" className="text-[48px] text-slate-300" />
            <p className="mt-3 text-[16px] font-bold text-[#0f172a]">No Health Concerns Found</p>
            <p className="text-[12.5px] text-[#64748b] mt-1">No health concern matching "{search}" was found.</p>
          </div>
        ) : (
              ...
              >
                ...
                <div className="flex aspect-square w-full items-center justify-center overflow-hidden rounded-xl bg-white p-1">
                  {hc.imageUrl ? (
                    ...
                    <Icon
                      name={hc.icon || 'health_and_safety'}
